...
- **分享集氣**:願望頁「分享集氣」按鈕產生 `worker /s/:id` 分享連結——爬蟲讀到該願望專屬 OG 卡(FB/LINE 不讀 SPA 的 `#wish-N` hash),真人點開自動跳回池子的該願望;行動裝置走系統原生分享,桌機複製連結。
...
- `worker/` — Cloudflare Worker(Hono)+ D1(migrations、175 個 vitest 測試)
