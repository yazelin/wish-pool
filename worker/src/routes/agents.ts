...
agents.post('/api/agent-tokens', async (c) => {
  ...
  await c.env.DB.prepare('INSERT INTO agent_tokens (token_hash, label, github_handle, created_at, created_ip_hash) VALUES (?, ?, ?, ?, ?)')
    .bind(hash, String(b.label ?? '').slice(0, 80) || null, String(b.github_handle ?? '').slice(0, 60) || null, Math.floor(Date.now() / 1000), fp).run()
